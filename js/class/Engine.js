  ...
  class Engine {
    constructor() {
      ...
      typingPad.onkeydown = (e) => {
        if (
          e.key === "Tab" ||
          ((e.metaKey || e.ctrlKey) && /[nqwefgplt]/.test(e.key))
        ) {
          // 消除一些默认浏览器快捷键的效果
          e.preventDefault();
        } else if (
    ...
    applyConfig() {
      ...
      $("input[type=checkbox]#bigCharacter").checked =
        this.config.isBigCharacter;
      let radioNodes = document.querySelectorAll(
      ...
      // Big Character Mode
      this.config.isBigCharacter
        ? enterBigCharacterMode()
        : leaveBigCharacterMode();
    ...
    prevChapter() {
      if (this.config.chapter !== 1) {
        if (this.config.articleType === ArticleType.word) {
          this.arrayWordDisplaying = this.arrayWordAll.slice(
            this.config.count * (this.config.chapter - 2),
            this.config.count * (this.config.chapter - 1)
          ); // 截取当前需要显示的数组段
          let arrayCurrentWord = this.arrayWordDisplaying.map((item) => {
            return item.word;
          }); // 取到英文，数组
    ...
    nextChapter() {
      if (this.config.chapter !== this.config.chapterTotal) {
        if (this.config.articleType === ArticleType.word) {
          this.arrayWordDisplaying = this.arrayWordAll.slice(
            this.config.count * this.config.chapter,
            this.config.count * (this.config.chapter + 1)
          ); // 截取当前需要显示的数组段
          let arrayCurrentWord = this.arrayWordDisplaying.map((item) => {
            return item.word;
      ...
      switch (this.config.articleType) {
        case ArticleType.character:
          this.currentOriginWords = this.config.isShuffle
            ? Utility.shuffle(article.content.split(""))
    ...
    compare() {
      ...
      for (let index = 0; index < arrayTyped.length; index++) {
        const currentCharacter = arrayTyped[index]; //自己打的尾字
        let originCharacter = arrayOrigin[index]; //系统出的尾字
        ...
        if (
          ...
          this.config.articleType === ArticleType.english
        ) {
          // 英文或单词时
        ...
        switch (item.type) {
          ...
          case ResultType.pending:
            if (index < result.length - 1) {
              // pending 不在最后一个，中间的 pending 都是错误的
      ...
      let untypedString = this.currentWords.substring(
        logLength - tempCharacterLength
      ); //未上屏字符
    ...
    showWrongWords() {
      ...
      let obj = [];
      let wrongWords = [];
      let words = this.config.wrongContent;
      ...
      //整理
      for (var key in obj) {
        let item = { word: key, num: obj[key] };
        wrongWords.push(item);
      }
      //排序
      this.config.wrongWords = wrongWords.sort(function (a, b) {
        return b.num - a.num;
      });
    ...
    finish() {
      ...
      this.record.speed = (
        (this.correctWordsCount / this.duration) *
        1000 *
        60
      ).toFixed(2);
      ...
      if (this.config.isAutoNext && !this.config.practice) {
        ...
        if (this.config.isAutoRepeat && !this.config.challenge) {
          ...
          if (this.config.repeatCountTotal > this.config.repeatCountCurrent) {
            // 还有重复次数
            if (this.config.isShuffleRepeat) {
              // 需要重复时乱序
              this.shuffleCurrent();
            } else {
              this.reset();
      ...
      if (!this.isStarted && !this.isFinished) {
        ...
        if (this.correctWordsCount) {
          this.record.codeLength = (
            allKeyCount / this.correctWordsCount
          ).toFixed(2);
        } else {
